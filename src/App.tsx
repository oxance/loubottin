        ...
        {session && <div className="max-w-screen-xl mx-auto my-8 px-8">
            <nav className="flex flex-row justify-between items-center w-full mb-8">
                ...
                <button className="button-ghost" onClick={() => supabase.auth.signOut()}>
                    <span className="hidden sm:inline">Sign Out </span><LogOut />
                </button>
            </nav>

            <div className="mb-8">
                <div className="relative flex flex-wrap pl-11 pr-3 rounded-2xl transition-all bg-slate-800 outline outline-2 outline-slate-700
                    ...
                    <input type="search" placeholder="Rechercher dans les contacts..." defaultValue={search.terms} onChange={e => searchTerms(e.target.value)} 
                        className="flex-1 px-0 py-2 min-w-64 peer !bg-transparent !outline-none" />
